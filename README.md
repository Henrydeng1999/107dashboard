...
当前比赛版本固定采用**单 Unix 账号模式**：Dashboard 只管理运行后端进程的当前平台账号，不实现学生注册、校园 SSO、账号切换或跨账号 Slurm 委托。比赛产品采用四个一级模块：作业提交管理、作业诊断自动报告、项目结果评价、AI 工作台；每个模块可拆分职责明确的子页面，避免把全部内容堆在一个长页面中。

本仓库当前交付物是**比赛演示 MVP，不是面向真实学生长期使用的生产客户端**。现阶段优先保证单账号完整流程、界面完成度、演示稳定性和可复核证据，不以生产级权限加固、统一认证、租户隔离、HTTPS、数据库加密、密钥轮换平台或长期服务治理作为验收目标，也不为追求形式上的生产标准统一改写已有运行目录权限。密钥不进入 Git/SQLite/API、Slurm 参数结构化校验、当前 owner 作业边界和 AI 只读边界仍是比赛阶段不可取消的最低安全底线。

## 目标功能

- 单 Unix 账号身份断言与基础权限控制
- 作业提交与参数校验
- 作业列表、状态和详情查看
- 实时或近实时日志查看
- 作业取消与作业克隆
- GPU、CPU、内存和运行时长统计
- 对接 Slurm 作业系统
- 基于可信证据的作业诊断与 Markdown 报告
- 项目级多实验对比与可选 AI 结果评价

当前本地产品工作台已实现三条新增链路：确定性作业诊断报告、多作业项目评价，以及只读 AI 工作台。AI Provider 采用 OpenAI 兼容 HTTPS 接口，密钥仅保存为用户私有运行时文件，不进入 SQLite、Git 或 API 响应；外部学校 Provider 仍需真实地址和凭证验收。
...
提交 `b253ac0` 已在 107 完成无写入演示发布验收，提交 `0d2c1d7` 又完成桌面、移动端和统一导航入口的整页排练。提交 `85c9646` 的 Native 全交互链也已在 107 通过：Job `24159` 正常完成并读取限量日志与 usage，Job `24160` 及克隆 Job `24161` 均由当前 UID 取消，三条提交、两条取消的幂等记录和审计链完整，且没有遗留活动作业。当前 Native-only 基础产品服务已部署；单账号试用进一步加入服务端登记的 CPU 完成、增量日志、受控失败和取消/克隆项目，提交时生成私有源码快照。下一步由项目成员在浏览器记录真实 Job 的端到端验收证据。

算力平台的实际环境检查结果见 [03 环境检查](docs/03-ENVIRONMENT_CHECK.md)。

当前目标是完成可演示的比赛 MVP，优先打通“填写作业参数 -> 提交 Slurm -> 查看状态与日志 -> 克隆作业 -> 查看资源统计”的完整闭环。生产级多用户委托、统一认证、权限基线加固、HTTPS、数据加密、容器化和长期服务治理放入赛后演进路线。

比赛开发继续使用 AI 辅助需求整理、代码编写、文档维护、测试和问题排查；产品侧则规划只读的诊断解释与项目结果评价。产品 AI 只能解释经过脱敏和结构化的证据，不得提交、取消、克隆作业，也不得覆盖 Slurm 事实或确定性规则结论。
