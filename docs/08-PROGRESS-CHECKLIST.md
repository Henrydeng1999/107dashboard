...
<div class="progress-summary" aria-label="当前项目摘要">
  ...
  <div class="progress-stat progress-stat-next"><strong>下一步</strong><span>实现受控 Native sbatch 提交</span></div>
</div>
...
<div class="notice-grid">
  <div class="notice notice-next"><strong>下一开发项</strong><span>实现固定分区/账户/QoS、受控脚本生成、参数数组 `sbatch`、Job ID 解析和事务化元数据保存，并先用最小资源 Fixture/注入测试验证。</span></div>
  <div class="notice notice-warning"><strong>暂时限制</strong><span>Native 只读已验收；真实提交、取消、克隆和日志继续关闭，下一阶段不会同时开放所有写操作。</span></div>
