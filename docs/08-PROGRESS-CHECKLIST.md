...
<div class="progress-summary" aria-label="当前项目摘要">
  ...
  <div class="progress-stat progress-stat-next"><strong>下一步</strong><span>Windows 客户端验收</span></div>
</div>
...
| <span class="status-badge status-done">✓</span> | 四模块统一工作台 | 可搜索且可整体收起的侧栏、折叠后悬停浮层预览、右上角账号状态、标签切换回顶、无圆框 SVG 导航和纯文字页脚 | 折叠栏仅保留等距一级图标，悬停展开不推动主内容；左栏滚动条隐藏但滚轮有效，一级/二级/footer 跳转重置滚动，无横向溢出、类型检查与发布构建通过 |
...
| <span class="status-badge status-active">→</span> | Windows 一键客户端 | WPF 单文件 EXE/便携 ZIP 使用公钥与 keyboard-interactive 登录，按版本上传 Linux 服务包、校验 SHA-256、启动 tmux、建立本地 SSH 隧道并在内置 WebView2 窗口加载主界面；支持服务日志、停止并断开、连接设置、最小化到托盘和退出清理；本地配置写入 EXE 同级 `data/`，成功连接后缓存目标、服务版本和端口；普通打开跳过完整环境/版本检查，失败自动回退；远端保留当前和上一个版本 | 客户端 9 项测试、Release 编译、内嵌载荷、本机启动和便携包检查通过；普通客户端部署跳过重复后端测试，保留发布包校验、`pip check` 和服务健康检查；真实 107 双因素认证、远程安装、WebView2 页面和托盘行为待账号持有人验收 |
...
<div class="notice-grid">
  <div class="notice notice-next"><strong>下一开发项</strong><span>由账号持有人使用 Windows 客户端完成真实 SSH 双因素认证、服务包上传、动态端口启动与内置 WebView2 页面及托盘链路验收；随后集中验收四模块 Native 流程和学校真实 AI Provider。</span></div>
