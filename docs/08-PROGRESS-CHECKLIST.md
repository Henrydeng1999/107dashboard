...
<div class="progress-summary" aria-label="当前项目摘要">
  ...
  <div class="progress-stat progress-stat-done"><strong>167 / 167</strong><span>后端测试通过</span></div>
  <div class="progress-stat progress-stat-done"><strong>通过</strong><span>前端类型检查与构建</span></div>
  <div class="progress-stat progress-stat-next"><strong>下一步</strong><span>107 提交无写入预检</span></div>
</div>
...
| <span class="status-badge status-done">✓</span> | 代码质量 | Ruff、pytest、npm audit | 后端 167 个 pytest 通过，目标 Ruff 通过；0 个生产依赖漏洞 |
...
<div class="notice-grid">
  <div class="notice notice-next"><strong>下一开发项</strong><span>在 107 运行不调用 `sbatch` 的提交预检；通过后等待一次新的明确授权，再执行最小真实作业验收。</span></div>
  ...
  <div class="notice notice-info"><strong>验证方式</strong><span>Fixture 与注入式 Native 回归用于本地稳定验证，107 脚本用于平台预检和真实验收：后端 pytest 167 passed、目标 Ruff、前端类型检查和生产构建通过。</span></div>
</div>
