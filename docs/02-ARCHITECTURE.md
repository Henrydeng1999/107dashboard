...
前端采用类似 Cloudflare 控制台的紧凑工作台布局作为参考，但不复制其品牌、图标或产品文案：固定侧栏、紧凑顶栏、中性黑背景、炭灰窗格、细边框、蓝色主操作和少量状态色。一级导航固定为“作业”“诊断与报告”“项目评价”“AI”，内容过多时拆分子页面，不允许通过无限增加页面高度解决信息密度问题。

桌面设计以浏览器**内容视口**而非物理显示器尺寸为准，需要预留浏览器标签栏、地址栏、窗口边框和 Windows 任务栏占用。基准视口为 `1600×800`（约 `2:1`），最低桌面验收视口为 `1440×720`。侧栏约占宽度 `12%–14%`，顶栏约 `56px`；主工作区优先使用约 `68/32` 的“大窗格 + 小窗格”双栏。页面根容器使用视口高度且禁止页面级滚动，长表格、日志、对话和列表只能在明确的内部窗格滚动。窄屏和移动端可以改为单栏并允许纵向滚动。
...
AI Chat 的证据范围由用户显式选择。历史作业只发送确定性结构化报告；Git 项目只发送不含路径、README 正文和文件内容的仓库名称、分支、HEAD、脏状态、变更计数与最近提交摘要，仓库工具同时限制到本次勾选的 opaque ID。提示词模板按 owner 存入 SQLite，可编辑或恢复默认，但只能补充分析侧重点；后端固定的只读安全 system prompt 始终优先，模板不能开放 Slurm 写工具或覆盖事实边界。
