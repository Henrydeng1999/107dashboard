...
这里的容器化只用于打包和运行 Dashboard 产品本身：前端、FastAPI 后端及其依赖、数据库和必要的反向代理。它的目标是避免每次部署都手动配置 Python、Node.js 和项目依赖。

Dashboard 容器不负责运行学生的 Docker 容器。学生作业仍然通过 Slurm 提交到计算节点，Dashboard 负责作业参数校验、提交、状态同步、日志读取和可视化。产品容器能否访问 Slurm，需要通过受控的命令适配器或宿主机代理解决，不等于需要访问 Docker socket。
