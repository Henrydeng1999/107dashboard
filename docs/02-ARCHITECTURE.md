...
当前已将有效 UID 用户名解析、部署 owner 精确匹配断言和 SQLite 作业元数据仓库接入应用装配与 Fixture 作业 service。SQLite 仓库保存 Dashboard ID、Slurm Job ID、结构化提交参数、最后状态和受控日志路径；跨 owner 查询按不存在处理，Slurm Job ID 全局唯一。Fixture 模拟提交和取消可在 service 重建后恢复，详情、日志、统计、取消和克隆均先经过统一 owner 可见性检查。Native API 仍继续 fail closed，下一阶段只在 107 有效 UID 校验通过后开放只读链路，并再次使用真实 Slurm owner 字段验收。
