...
`scripts/run-native-live-interaction.py` 以真实 FastAPI HTTP 路由为边界，要求四个 Native 能力门全部开启且当前来源为未降级的 `native`。脚本固定提交三个最小作业：完成样例使用 `python3 --version` 和 `1 CPU / 512 MiB / 0 GPU / 1 分钟`；控制样例及其克隆使用 `python3 -m timeit -n 1000000000 pass` 和 `1 CPU / 512 MiB / 0 GPU / 2 分钟`。完成样例必须到达 `COMPLETED/0:0`，每个日志流最多读取 4096 字节但不在报告中回显正文；控制样例和克隆必须分别取消。

验收结果还必须证明列表与摘要数量一致、三个 Slurm Job ID 互不相同、三条提交和两条取消幂等记录均为 `SUCCEEDED`，且三个 `SBATCH_ACCEPTED` 与两个 `SCANCEL_ACCEPTED` 审计事件存在。脚本使用固定确认短语和一次性回执防止误运行；异常时只尝试取消本次返回并记录的活动 Job。该协议通过本地注入式 Slurm 回归后仍需 107 实测，不能以本地伪命令结果代替平台证据。

2026-07-19，提交 `85c9646` 按该协议在 107 通过。Job `24159` 达到 `COMPLETED/0:0` 并返回限量日志与 usage；Job `24160` 和克隆 Job `24161` 均成功取消。报告确认 `submitted_jobs=3`、`cancelled_jobs=2`、五条幂等记录成功、审计链存在、日志正文未输出，最终 `squeue` 为空。该证据批准进入真实 Web 服务与浏览器交互联调，不改变单 Unix owner、窄命令策略和回退期间禁止写入的边界。

### 单账号登记测试项目

`GET /api/projects` 返回服务端登记项目的 ID、名称、说明、入口文件名、预期终态、受控命令引用和默认资源，不返回项目根目录或源码内容。浏览器提交 `python3 @project/<id>`；Native 后端仅在 `TEST_PROJECT_DIRECTORY` 中解析该 ID，并在调用 `sbatch` 前生成私有源码快照。项目文件必须属于后端有效 UID、不可被组或其他用户写入、不是符号链接且入口不超过 64 KiB。未知或不安全项目映射为脱敏 `422`，不得创建工作目录或调用 Slurm。

`GET /api/runtime` 返回配置目标 `data_source`、当前服务来源 `serving_source`、`read_only`、`degraded`、演示回退状态，以及列表、详情、usage、提交、取消、克隆、日志能力。前端以此隐藏未开放入口；该响应不包含 Unix UID、用户名、路径或命令。`data_source=native` 但 `serving_source=fixture_fallback` 表示当前因 Slurm 查询故障展示脱敏演示数据，此时 `submit/cancel/clone=false`。

## 诊断、项目评价与 AI 工作台 API

- `GET /api/reports` 与 `GET /api/reports/{job_id}` 基于当前 owner 可见作业生成确定性报告。响应明确区分 Slurm、usage 和持久化 metadata 证据，并返回规则版本、健康分、发现和建议；AI 不参与事实和分数生成。
- `GET/POST /api/evaluation-projects` 与 `GET /api/evaluation-projects/{project_id}` 管理 SQLite 中的单账号评价项目。创建时所有 `job_ids` 必须仍对当前 owner 可见；评价返回关联作业、规则分数、等级、证据覆盖率和缺口建议。
- `GET /api/ai/providers` 与 `PUT /api/ai/providers/{provider_id}` 管理 OpenAI 兼容 HTTPS Provider。Provider ID 只允许字母、数字、下划线和连字符。API Key 是只写字段：原文只以用户私有文件保存，目录权限为 `700`、文件权限为 `600`，SQLite 和响应仅保留末四位提示。
- `POST /api/ai/chat` 只把请求中明确列出的 owner 可见作业转换为结构化诊断报告后发送给已配置 Provider，不发送原始日志，也不向模型暴露提交、取消或克隆能力。Provider 未配置返回 `409`，调用故障返回脱敏 `503`。
- `GET /api/ai/templates` 返回内置只读提示词，`GET /api/ai/calls` 返回当前 owner 的最近调用状态与截断预览，不返回密钥或完整底层错误。

这些产品 API 使用现有单 Unix 账号边界。外部模型回答是辅助解释，不得覆盖 Slurm 状态、退出码、确定性报告分数或项目证据覆盖率。
