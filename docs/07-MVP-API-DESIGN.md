...
比赛原型选用单 Unix 账号身份边界。Native adapter 的 `squeue` 和 `sacct` 只查询有效 UID 对应用户，service 再按 Slurm `user` 字段过滤；详情和资源统计先从该 owner 的可见列表定位 Job ID，其他 owner 统一按不存在处理。Native 的 `POST /api/jobs`、取消、克隆和日志仍返回脱敏 `503`，不能通过环境布尔值或 HTTP 输入开启。

Native 提交的独立安全 service 已在本地实现并通过注入测试，但没有连接到上述路由。它使用 `python`/`python3` 命令白名单、受限参数、固定 `Students + stu + qos_stu_default`、QoS 资源上限、受控目录与脚本、`sbatch` 参数数组、`--parsable` Job ID 解析、本地回执、owner 元数据和脱敏审计；实际调用还要求显式授权对象。平台验收通过不自动开放 API，完成重复请求、并发门禁和 API 回归前契约保持不变。

2026-07-18，提交 `88a0147` 的一次性入口在 107 完成真实验收。Job `24011` 使用 `python3 --version` 和 `1 CPU / 512 MiB / 0 GPU / 1 分钟`，最终为 `COMPLETED`、退出码 `0:0`；Dashboard 元数据、回执与 `PREPARED -> SUCCEEDED` 审计均存在。该证据只验证底层受控提交链，不改变 API 契约：Native `POST /api/jobs` 仍返回 `503`，也未开放取消、克隆或日志。
