...
Native 提交的独立安全 service 已在本地实现并通过注入测试，但没有连接到上述路由。它使用 `python`/`python3` 命令白名单、受限参数、固定 `Students + stu + qos_stu_default`、QoS 资源上限、受控目录与脚本、`sbatch` 参数数组、`--parsable` Job ID 解析、本地回执、owner 元数据和脱敏审计；实际调用还要求显式授权对象。平台预检和最小真实作业验收完成前，API 契约不变。
