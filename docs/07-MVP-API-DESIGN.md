...
比赛原型选用单 Unix 账号身份边界：Native 实现必须从 FastAPI 进程的有效 UID 解析用户名，并将 `DASHBOARD_OWNER` 作为启动一致性断言；任何 HTTP 输入都不能选择用户。当前有效 UID 解析、owner 精确匹配断言和 owner 限定的 SQLite 元数据仓库已经接入应用装配和 Fixture service。模拟提交与取消会持久化，service 重建后恢复；详情、日志、统计、取消和克隆对其他 owner 均统一返回 `404`。Native 代码门禁仍不会自动开放，下一阶段需单独启用只读 adapter 并在 107 复核 Slurm user 字段。
