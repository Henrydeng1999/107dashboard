...
比赛原型选用单 Unix 账号身份边界。Native adapter 的 `squeue` 和 `sacct` 只查询有效 UID 对应用户，service 再按 Slurm `user` 字段过滤；详情和资源统计先从该 owner 的可见列表定位 Job ID，其他 owner 统一按不存在处理。Native 的取消、克隆和日志仍返回脱敏 `503`。`POST /api/jobs` 默认同样关闭，只有部署方设置 `NATIVE_SUBMISSION_ENABLED=true` 且 UID/owner 校验通过时才开放，HTTP 输入不能覆盖该开关或选择用户。
