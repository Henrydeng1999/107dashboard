...
比赛原型选用单 Unix 账号身份边界。Native adapter 的 `squeue` 和 `sacct` 只查询有效 UID 对应用户，service 再按 Slurm `user` 字段过滤；详情、资源统计和日志先从该 owner 的可见列表定位 Job ID，其他 owner 统一按不存在处理。Native 的取消和克隆仍返回脱敏 `503`。提交与日志默认关闭，分别只有部署方设置 `NATIVE_SUBMISSION_ENABLED=true`、`NATIVE_LOGS_ENABLED=true` 且 UID/owner 校验通过时才开放；HTTP 输入不能覆盖开关或选择用户。
