...
比赛原型选用单 Unix 账号身份边界：Native 实现必须从 FastAPI 进程的有效 UID 解析用户名，并将 `DASHBOARD_OWNER` 作为启动一致性断言；任何 HTTP 输入都不能选择用户。当前有效 UID 解析、owner 精确匹配断言和 owner 限定的 SQLite 元数据仓库已经实现并通过测试。每个作业操作仍需以 Slurm 返回的 user 或持久化 owner 复核所有权，这些组件也尚未接入应用启动和 service，因此代码门禁不会自动开放。
