...
- 三个主页面形成统一导航：作业提交管理、作业诊断自动报告、项目 AI 结果评价；
...
- 按已确认设计实现作业提交管理、作业诊断自动报告和项目 AI 结果评价三个主页面；
...
已确定答案：单 Unix 账号、三个主页面、Slurm 事实优先、规则诊断先行、AI 只读且手动触发。其余推荐默认采用 OpenAI-compatible 私有模型、通用长格式 `metrics.jsonl`、Markdown 报告和 Three.js 延后；首批验收尽量同时包含一个 CPU 项目和一个 GPU 项目。
