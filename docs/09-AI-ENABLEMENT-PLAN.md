...
项目按约定输出 `metrics.jsonl` 或 `metrics.csv`，后端进行受限解析，前端展示 Loss、Accuracy、Learning Rate 和资源趋势。AI 只能解释已解析的数据，不解析图像像素猜测数值。
...
- 支持一个受控示例训练项目输出结构化 metrics；
- 后端解析 metrics，前端展示至少一条 Loss 曲线；
...
- 不允许 AI 自动执行修复或重新提交；
- 不允许用户输入任意模型 Endpoint 或 API Key；
- 不读取任意服务器路径或其他用户产物；
- 不加载 PyTorch pickle checkpoint；
- 不做任意格式科研数据理解；
- 不以 Three.js 动态背景作为阻塞项；
- 不承诺生产级多用户、校园 SSO 或跨账号 Slurm 委托。
...
- API Key 只存在于部署环境或密钥服务，不进入 SQLite、日志、前端或 Git；
- Endpoint 和模型只由管理员配置；
...
视觉升级重点应是层级、密度、状态反馈、排版和一致组件，而不是堆叠发光卡片。动态背景只允许低对比度、可关闭、尊重 `prefers-reduced-motion`，不得影响日志阅读或低性能终端。
...
2. **提交**：选择受控训练示例，配置最小资源并提交；
3. **运行**：展示实时状态和增量日志，不等待长训练；
4. **结果**：打开预置完成作业，展示真实 Loss 曲线和资源统计；
...
3. 比赛阶段只做单 Unix 账号演示，还是必须展示多用户隔离；
4. 首个指标协议面向哪类任务：PyTorch 训练、通用 CSV，或两者；
5. 报告只需 Markdown 预览与下载，还是必须生成 PDF；
6. 比赛评分更偏技术创新、视觉展示，还是平台安全与工程完整度。

推荐默认答案：单账号演示、OpenAI-compatible 私有模型、`metrics.jsonl`、Markdown 报告、规则诊断先行、LLM 手动触发、Three.js 延后。这个组合能在不牺牲现有安全底座的前提下，用最短路径形成真正可展示的 AI+HPC 闭环。
