...
本章当前记录的是比赛演示部署，不是面向真实学生的生产客户端部署规范。部署验收关注代码可追溯、单账号流程可运行、真实 Slurm 事实可复核和演示后无测试作业遗留；不要求在本阶段统一重写现有运行目录、SQLite、日志或普通配置文件权限，也不引入生产 IAM、HTTPS、静态数据加密、密钥轮换或合规审计。密钥不入 Git/API、Slurm owner/参数边界和 AI 只读边界仍必须保留。
...
以下条目用于防止比赛演示直接泄密、越权或误操作 Slurm，不应解读为完整的生产安全方案。真实学生客户端上线前仍需另行完成认证、租户隔离、权限基线、HTTPS、密钥治理、审计和备份设计。
